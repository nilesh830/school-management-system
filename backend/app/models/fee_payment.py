...
class FeePayment(db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            "payment_method IN ('cash', 'bank_transfer', 'cheque', 'online')", name="ck_fee_payments_payment_method"
        ),
    )

    # Relationships
    fee_record = db.relationship("FeeRecord", backref=db.backref("payments", lazy="dynamic"))
    collected_by_user = db.relationship(
        "User", foreign_keys=[collected_by], backref=db.backref("collected_payments", lazy="dynamic")
    )
