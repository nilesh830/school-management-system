...
class Discount(db.Model):
    ...
    __table_args__ = (
        CheckConstraint(
            "discount_type IN ('scholarship', 'sibling', 'staff', 'custom')",
            name='ck_discounts_discount_type'
        ),
        CheckConstraint(
            'amount > 0',
            name='ck_discounts_amount_positive'
        ),
    )
    ...
    approved_by_user = db.relationship(
        ...
        backref=db.backref('approved_discounts', lazy='dynamic')
    )
    ...
    def __repr__(self):
        return (
            f'<Discount id={self.id} student_id={self.student_id} '
            f'type={self.discount_type!r} amount={self.amount}>'
        )
