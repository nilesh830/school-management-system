...
def export_attendance_report():
    ...
    if fmt == "pdf":
        content, err = ReportService.export_attendance_pdf(section_id, from_date, to_date)
        if err:
            return error_response(err["message"], status=err.get("status", 400))
        return _file_response(content, _PDF_MIMETYPE, f"attendance_report_{section_id}.pdf")

    content, err = ReportService.export_attendance_excel(section_id, from_date, to_date)
    if err:
        return error_response(err["message"], status=err.get("status", 400))
    return _file_response(content, _XLSX_MIMETYPE, f"attendance_report_{section_id}.xlsx")
...
def export_grades_report():
    ...
    if fmt == "pdf":
        ...
        if err:
            return error_response(err["message"], status=err.get("status", 400))
        return _file_response(content, _PDF_MIMETYPE, f"grades_report_{exam_id}.pdf")

    content, err = ReportService.export_grades_excel(exam_id, section_id)
    if err:
        return error_response(err["message"], status=err.get("status", 400))
    return _file_response(content, _XLSX_MIMETYPE, f"grades_report_{exam_id}.xlsx")
