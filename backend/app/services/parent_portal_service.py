...
class ParentPortalService:
    ...
    def get_child_grades(parent_id: int, child_id: int) -> dict:
        ...
        exam_ids = (
            get_db()
            .query(ExamResult.exam_id)
            .filter(ExamResult.student_id == child_id)
            .distinct()
            .all()
        )
    ...
    def get_child_fees(parent_id: int, child_id: int) -> dict:
        ...
        fee_records = (
            get_db()
            .query(FeeRecord)
            .filter(FeeRecord.student_id == child_id)
...
class LeaveService:
    ...
    def submit(parent_id: int, data: dict) -> tuple:
        ...
        ss = get_db().query(StudentSection).filter_by(
            student_id=child_id, is_current=True
        ).first()
        if ss:
            section = get_db().query(Section).filter_by(id=ss.section_id).first()
            if section and section.class_teacher_id:
                teacher = get_db().query(Teacher).filter_by(
                    id=section.class_teacher_id
                ).first()
                if teacher:
    ...
    def review(leave_id: int, reviewer_user_id: int, status: str, remarks: str = None) -> tuple:
        ...
        notif_body = (
            f"Your leave application from {leave.from_date} to {leave.to_date} "
            f"has been {status}."
        )
        if remarks:
            notif_body += f" Remarks: {remarks}"
        NotificationService.create(
            parent_user_id, 'leave_update', notif_title, notif_body,
...
class NotificationService:
    ...
    def mark_all_read(user_id: int) -> int:
        count = get_db().query(Notification).filter_by(user_id=user_id, is_read=False)\
            .update({'is_read': True})
        get_db().commit()
        return count
...
class MessageService:
    ...
    def list_threads(user_id: int, role: str) -> list:
        ...
        if role == 'parent':
            ...
            if not parent:
                return []
            threads = get_db().query(MessageThread).filter_by(
                parent_id=parent.id
        ...
        for t in threads:
            d = t.to_dict()
            unread = t.messages.filter_by(is_read=False).filter(
