...
def _validate_conflicts(section_id: int, teacher_id: int, day_of_week: int,
                         period_no: int, exclude_id: int = None):
    """
    ...
    q_section = get_db().query(Timetable).filter_by(
        section_id=section_id, day_of_week=day_of_week, period_no=period_no
    )
    if exclude_id:
    ...
    q_teacher = get_db().query(Timetable).filter_by(
