...
class SectionService:
    ...
    def get_by_id(section_id: int):
        ...
        student_count = (
            ...
            .filter_by(section_id=section_id, is_current=True)
            .count()
        )
    ...
    def create(data: dict):
        ...
        existing = get_db().query(Section).filter_by(
            name=name, class_id=class_id, is_active=True
        ).first()
        if existing:
            return None, {
    ...
    def update(section_id: int, data: dict):
        ...
        if 'name' in data:
            ...
            duplicate = get_db().query(Section).filter_by(
                name=new_name, class_id=class_id, is_active=True
            ).first()
    ...
    def enroll_student(section_id: int, student_id: int, academic_year_id: int):
        ...
        already = get_db().query(StudentSection).filter_by(
            student_id=student_id, section_id=section_id, is_current=True
        ).first()
        if already:
            return None, {
        ...
        old = get_db().query(StudentSection).filter_by(
    ...
    def unenroll_student(section_id: int, student_id: int):
        ...
        """
        enrollment = get_db().query(StudentSection).filter_by(
            section_id=section_id, student_id=student_id, is_current=True
        ).first()
        if not enrollment:
            return False, {
