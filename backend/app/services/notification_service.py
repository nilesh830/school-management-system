...
class NotificationService:
    ...
    @staticmethod
    def create(user_id: int, type: str, title: str, body: str, reference_id: int = None, reference_type: str = None):
    ...
    def get_for_user(user_id: int, unread_only: bool = False, limit: int = 50):
        ...
        query = get_db().query(Notification).filter(Notification.user_id == user_id)
        if unread_only:
    ...
    def mark_read(notification_id: int, user_id: int):
        ...
        notif = get_db().query(Notification).filter_by(id=notification_id, user_id=user_id).first()
        if not notif:
    ...
    def mark_all_read(user_id: int):
        ...
        get_db().query(Notification).filter_by(user_id=user_id, is_read=False).update({"is_read": True})
        get_db().commit()
