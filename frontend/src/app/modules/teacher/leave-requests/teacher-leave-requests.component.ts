...
/**
 * Teacher · Leave Requests — review student leave applications.
 *
 * Backend allows teacher + admin to list all leave applications and approve /
 * reject them (GET /leave-applications/all, PUT /leave-applications/:id/review).
 * Teachers cannot submit leaves (parent-only) so there is no create action here.
 */
@Component({
  ...
  template: `
    ...
    <div class="card">
      ...
      @if (!loading && leaves.length === 0) {
        <p-message severity="info" text="No leave applications found for the selected filter." styleClass="w-full" />
      }
          ...
          <ng-template pTemplate="body" let-leave>
            <tr>
              ...
              <td class="text-center">
                <p-tag [value]="getStatusLabel(leave.status)" [severity]="getStatusSeverity(leave.status)" />
              </td>
              <td class="text-center">
                @if (leave.status === 'pending') {
                  <div class="flex gap-1 justify-content-center">
                    <p-button
                      icon="pi pi-check" severity="success" [rounded]="true" [text]="true"
                      size="small" pTooltip="Approve" (onClick)="openReview(leave, 'approved')"
                    />
                    <p-button
                      icon="pi pi-times" severity="danger" [rounded]="true" [text]="true"
                      size="small" pTooltip="Reject" (onClick)="openReview(leave, 'rejected')"
                    />
                  </div>
                } @else {
    ...
    >
      ...
      <form [formGroup]="reviewForm" (ngSubmit)="submitReview()">
        <div class="field">
          ...
          <textarea
            pInputTextarea formControlName="remarks" rows="3"
            placeholder="Optional remarks for the parent..." class="w-full"
          ></textarea>
        </div>

        <div class="flex justify-content-end gap-2 mt-3">
          <p-button label="Cancel" severity="secondary" [text]="true" type="button" (onClick)="closeReview()" />
          <p-button
...
export class TeacherLeaveRequestsComponent implements OnInit {
  ...
  reviewForm = this.fb.group({ remarks: [''] });
  ...
  loadLeaves(): void {
    ...
    this.http.get('/api/v1/leave-applications/all', { params }).subscribe({
      next: (res: any) => {
        this.leaves = res.data?.leave_applications ?? res.data?.leaves ?? res.data ?? [];
        this.loading = false;
      },
      error: () => {
        this.loading = false;
  ...
  getTypeLabel(type: string): string {
    const map: Record<string, string> = { sick: 'Sick', family: 'Family', personal: 'Personal', other: 'Other' };
